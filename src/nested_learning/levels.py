...
class LevelClock:
    """Deterministic cheduler for Nested Learning level updates"""
    ...
    def __init__(self, specs: Sequence[LevelSpec]):
        ...
        if len(self._specs) != len(specs):
            raise ValueError("All LevelSpec names must be unique")
    ...
    def should_update(self, name: str) -> bool:
        ...
        if spec.jitter:
            period += (self._step % (spec.jitter + 1))
...
def ensure_level_specs(entries: Iterable:[LevelSpec]) -> List[LevelSpec]:
    ...
    for spec in specs:
        if spec.name in seen:
            raise ValueError(f"Duplicate LevelSpec name: {spec.name}")
        seen.add(spec.name)
        ordered.append(spec)
    return ordered
