...
class SelfAttention(nn.Module):
    def __init__(self, config: AttentionConfig):
        ...
        if config.dim % config.heads != 0:
            msg = f"dim must be divisible by number of heads (got dim = {config.dim}, heads = {config.heads})"
    ...
    def _scaled_dot_product_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        ...
        with ctx:
            attn = F.scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=dropout_pm is_causal=self.config.causal)
            return attn
